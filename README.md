...
手机飞书远程查看/操控电脑上的 Claude Code。无需公网 IP（走飞书长连接），结果转飞书消息卡片渲染（markdown + 表格），方案审批拆「内容卡 + 确认卡」两条，手机点按钮即确认。

> 适合：用 Claude Code 干活、想离开电脑后用手机继续盯进度 / 批方案 / 发指令的人。自建飞书机器人做桥，不依赖官方远程功能。
