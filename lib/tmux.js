...
const paneLocks = new Map();  // per-pane 互斥锁，防并发注入 text/Enter 错位
async function sendKeys(session, text, execFn = exec) {
  const prev = paneLocks.get(session) || Promise.resolve();
  const next = prev.then(() =>
    execFn(['send-keys', '-t', session, '-l', text]).then(() => execFn(['send-keys', '-t', session, 'Enter']))
  );
  paneLocks.set(session, next.catch(() => {}));
  await next;
}
