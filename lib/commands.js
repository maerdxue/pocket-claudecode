...
async function handleMessage(msg, d) {
  ...
  if (d.myOpenId && openId && openId !== d.myOpenId) return;
...
async function cmdOpen(d, replyChatId, arg) {
  ...
  if (/^\d+$/.test(arg)) {
    ...
    const matches = list.filter(s => s.name === arg);
  ...
  const chatId = await d.createGroup(groupName, d.myOpenId);
...
async function handleCardAction(action, d) {
  ...
  if (d.myOpenId && openId && openId !== d.myOpenId) return;  // 白名单，同 handleMessage
  const sessionId = registry.findByChatId(d.reg, chatId);
  ...
  try {
    ...
    if (e.message === 'not-in-tmux') { await d.send(chatId, notInTmuxMsg()); return; }
    throw e;
  }
