...
// 清理 /status 截屏里的 CC TUI 装饰行（状态栏/accept/agent/分隔线/输入框/窗口名），只留实质内容
function cleanStatusScreen(raw) {
  if (!raw) return raw;
  return raw.split('\n')
    .filter(line => {
      const t = line.trim();
      if (!t) return true;                       // 保留空行(段落分隔)
      if (/⎇/.test(t)) return false;             // 状态栏(git分支+模型+token%)
      if (/⏵/.test(t)) return false;             // accept edits / 模式行
      if (/^[⯑]/.test(t)) return false;          // main agent 标记
      if (/^[◯●○]/.test(t)) return false;        // agent 列表项
      if (/─{6,}/.test(t)) return false;          // 长分隔线
      if (/─{2,}\s*$/.test(t) && t.length < 40) return false;  // 窗口名行(短+结尾──)
      if (/^❯/.test(t)) return false;            // 输入提示符
      return true;
    })
    .join('\n')
    .replace(/\n{3,}/g, '\n\n')
...
async function handleMessage(msg, d) {
  ...
  if (!isGroup && isCmd) {
    ...
    if (cmd === 'help') return d.send(chatId, [
      ...
      '群里: /status /history [N] /close 是 relay 命令；其他 / 命令(/goal /compact 等)和文本直接注入 CC',
  ...
  if (isCmd) {
    ...
    if (cmd === 'status') {
      ...
      return d.send(chatId, truncate(cleanStatusScreen(screen) || '(无画面)', 30000, '…(已截断，/history 看全)'));
    }
    // 其他 / 命令(如 /goal /compact /clear) 不拦截，落到下面注入 CC
  }
...
module.exports = { handleMessage, handleCardAction, cmdOpen, formatList, notRunningMsg, notInTmuxMsg, orderedSessions, cleanStatusScreen };
